...
image::/img/backup_restore/backup/backup.png[]
...
image::/img/backup_restore/backup/schedule.png[]
...
image::/img/backup_restore/backup/storageLocation.png[]
...
. *Credential Secret* (optional): If you need to use the AWS Access keys Secret keys to access s3 bucket, create a secret with your credentials with keys and the directives `accessKey` and `secretKey`. It can be in any namespace. An example secret is <<example-credentialsecret,here.>> This directive is unnecessary if the nodes running your operator are in EC2 and set up with IAM permissions that allow them to access S3, as described in <<iam-permissions-for-ec2-nodes-to-access-s3,this section.>> The Credential Secret dropdown lists the secrets in all namespaces.
