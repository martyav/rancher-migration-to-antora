...
. From the main navigation bar, choose menu:Resources[Workloads.] Find the `cattle-system` namespace. Open the `rancher` workload by clicking its link.
...
You can enable Rancher's built in log collection and shipping for the cluster to ship the audit and other services logs to a supported collection endpoint. See xref:../../../../explanations/integrations-in-rancher/logging/logging.adoc[Logging] for details.
