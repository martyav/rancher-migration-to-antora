...
Rancher uses LDAP to communicate with the Active Directory server. The authentication flow for Active Directory is therefore the same as for the xref:../../../../../reference-guides/configure-openldap/configure-openldap.adoc[OpenLDAP authentication] integration.

[NOTE]
====


Before you start, please familiarise yourself with the concepts of link:../about-authentication.adoc#external-authentication-configuration-and-principal-users[External Authentication Configuration and Principal Users].
====
...
| This settings defines whether Rancher should resolve nested group memberships. Use only if your organization makes use of these nested memberships (i.e., you have groups that contain other groups as members. We advise avoiding nested groups when possible).
|===
...
The https://manpages.ubuntu.com/manpages/focal/en/man1/ldapsearch.1.html[`ldapsearch`] tool allows you to query your AD server to learn about the schema used for user and group objects.
