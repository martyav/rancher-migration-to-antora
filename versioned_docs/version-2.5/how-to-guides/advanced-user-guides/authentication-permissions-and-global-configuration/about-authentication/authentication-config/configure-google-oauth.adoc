...
Within Rancher, only administrators or users with the *Manage Authentication* xref:../../manage-role-based-access-control-rbac/global-permissions.adoc[global role] can configure authentication.
...
. Go to the https://admin.google.com/AdminHome?chromeless=1#OGX:ManageOauthClients[*Manage OAuth Client Access* page.]
...
. Sign into Rancher using a local user assigned the xref:../../manage-role-based-access-control-rbac/global-permissions.adoc[administrator] role. This user is also called the local principal.
. From the *Global* view, click menu:Security[Authentication] from the main menu.
