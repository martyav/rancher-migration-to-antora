...
| monitoring-ui-view
| _Available as of Monitoring v2 14.5.100+_ Provides read-only access to external Monitoring UIs by giving a user permission to list the Prometheus, Alertmanager, and Grafana endpoints and make GET requests to Prometheus, Grafana, and Alertmanager UIs through the Rancher proxy.
|===
...
To see the Grafana UI, install `rancher-monitoring`. Then go to the *Cluster Explorer.* In the top left corner, click menu:Cluster Explorer[Monitoring.] Then click **Grafana.+++<figcaption>+++Cluster Compute Resources Dashboard in Grafana+++</figcaption>+++
