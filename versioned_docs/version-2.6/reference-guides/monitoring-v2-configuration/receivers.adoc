...
Tab Rancher v2.6.5+::
+
1. In the Rancher UI, go to the cluster where you want to install rancher-alerting-drivers and click **Apps**. 1. Click the **Alerting Drivers** app. 1. Click the **Helm Deploy Options** tab. 1. Select the **Teams** option and click **Install**. 1. Take note of the namespace used as it will be required in a later step. 

Tab Rancher before v2.6.5::
+
1. In the Rancher UI, go to the cluster where you want to install rancher-alerting-drivers and click **Apps & Marketplace**. 1. Click the **Alerting Drivers** app. 1. Click the **Helm Deploy Options** tab. 1. Select the **Teams** option and click **Install**. 1. Take note of the namespace used as it will be required in a later step.
====
...
If you have a cluster that uses SMS or Microsoft Teams alerts, and upgrade the Monitoring chart from 100.1.3+up19.0.3 to 100.2.0+up40.1.2, you may no longer receive alerts. This is due to a change to an upstream change to https://github.com/prometheus-operator/prometheus-operator/[Prometheus Operator] that helps prevent naming collisions.

You must manually edit the configuration map to continue to receive alerts:

. Click on *☰* and select the relevant cluster.
. Select *More Resources* > *Core* > *ConfigMaps*.
. Find the row corresponding to the ConfigMap for *Sachet* (for SMS alerts) or *prom2teams* (for Microsoft Teams alerts). Click the *⋮* and select *Edit YAML*.
. Update the delimiters in the receiver's `name` field, replacing `-` characters with `/`, according to the following scheme: `<namespace>\<alertManagerConfig>\<receiverName>`. For example, if the receiver's name is `default-amc-test-prom2sms`, update it to `default/amc-test/prom2sms`.
. Click *Save*.
. Redeploy the workloads associated with Alertmanager and Sachet or prom2teams, to ensure that they receive the updated configuration.
