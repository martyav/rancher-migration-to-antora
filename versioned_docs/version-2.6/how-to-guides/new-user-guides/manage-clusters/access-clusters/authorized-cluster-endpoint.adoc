...
If admins have link:../../../../reference-guides/about-the-api/api-tokens.adoc#disable-tokens-in-generated-kubeconfigs[kubeconfig token generation turned off], the kubeconfig file requires xref:./authorized-cluster-endpoint.adoc[rancher cli] to be present in your PATH.
