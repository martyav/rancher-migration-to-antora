...
* Use dedicated nodes for each role. This ensures resource availability for the components needed for the specified role. It also strictly isolates network traffic between each of the roles according to the link:../node-requirements-for-rancher-managed-clusters.adoc#networking-requirements[port requirements].
 ...
 ** Docs on https://etcd.io/docs/v3.5/faq/#what-is-failure-tolerance[etcd v3.5], for clusters running Kubernetes v1.22 and later.
 ** Docs on https://etcd.io/docs/v3.4/faq/#what-is-failure-tolerance[etcd v3.4] for clusters running Kubernetes v1.18 to Kubernetes v1.21.
