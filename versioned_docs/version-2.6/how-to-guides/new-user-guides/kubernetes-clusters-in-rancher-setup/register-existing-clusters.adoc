...
If you are registering a K3s cluster, make sure the `cluster.yml` is readable. It is protected by default. For details, refer to <<configuring-a-k3s-cluster-to-enable-registration-in-rancher,Configuring a K3s cluster to enable importation to Rancher.>>
...
GKE clusters must be running in Standard mode. GKE Autopilot clusters aren't supported. See https://cloud.google.com/kubernetes-engine/docs/resources/autopilot-standard-feature-comparison[Compare GKE Autopilot and Standard] for more information about the difference between GKE modes.
