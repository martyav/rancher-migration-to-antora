...
. In the *Project Release Namespace Project ID* field, the `System Project` is used as the default but can be overridden with another project with similarly <<ensure-the-cattle-monitoring-system-namespace-is-placed-into-the-system-project-or-a-similarly-locked-down-project-that-has-access-to-other-projects-in-the-cluster,limited access>>. Project IDs can be found with the following command run in the local upstream cluster:
