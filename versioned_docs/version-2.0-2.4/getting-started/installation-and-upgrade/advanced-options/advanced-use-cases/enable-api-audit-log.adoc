...
* link:../../../../reference-guides/single-node-rancher-in-docker/advanced-options.adoc#api-audit-log[Docker Install]
* link:../../../../reference-guides/installation-references/helm-chart-options.adoc#api-audit-log[Kubernetes Install]
...
`3` - Log event metadata, request body, and response body. Each log transaction for a request/response pair uses the same `auditID` value. +
 +
See <<audit-log-levels,Audit Level Logging>> for a table that displays what each setting logs.
...
The following table displays what parts of API transactions are logged for each <<api-audit-log-options,`AUDIT_LEVEL`>> setting.
...
. From the context menu, select menu:Cluster: local[System].
. From the main navigation bar, choose menu:Resources[Workloads.] (In versions before v2.3.0, choose *Workloads* on the main navigation bar.) Find the `cattle-system` namespace. Open the `rancher` workload by clicking its link.
. Pick one of the `rancher` pods and select menu:&#8942;[View Logs].
. From the *Logs* drop-down, select `rancher-audit-log`.

==== Shipping the Audit Log

You can enable Rancher's built in log collection and shipping for the cluster to ship the audit and other services logs to a supported collection endpoint. See xref:../../../../explanations/integrations-in-rancher/cluster-logging/cluster-logging.adoc[Rancher Tools - Logging] for details.
