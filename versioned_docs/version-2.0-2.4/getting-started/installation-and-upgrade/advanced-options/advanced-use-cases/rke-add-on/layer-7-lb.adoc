...
* https://kubernetes.github.io/ingress-nginx/[NGINX Ingress controller (HTTP)]
...
image::/img/ha/rancher2ha-l7.svg[Rancher HA]
...
| `Host`
| FQDN used to reach Rancher.
| To identify the server requested by the client.

| `X-Forwarded-Proto`
| `https`
| To identify the protocol that a client used to connect to the load balancer. +
 +
*Note:* If this header is present, `rancher/rancher` does not redirect HTTP to HTTPS.

| `X-Forwarded-Port`
| Port used to reach Rancher.
| To identify the protocol that client used to connect to the load balancer.
...
* xref:../../../../../how-to-guides/new-user-guides/infrastructure-setup/amazon-elb-load-balancer.adoc[Amazon ELB configuration]
* xref:../../../../../how-to-guides/new-user-guides/infrastructure-setup/nginx-load-balancer.adoc[NGINX configuration]
 ...
 ** xref:../cluster-yml-templates/node-externalssl-certificate.adoc[Template for self-signed certificate +
`3-node-externalssl-certificate.yml`]installation/options/cluster-yml-templates/3-node-externalssl-certificate)
 ** xref:../cluster-yml-templates/node-externalssl-recognizedca.adoc[Template for certificate signed by recognized CA +
...
When using RHEL/CentOS, the SSH user can't be root due to https://bugzilla.redhat.com/show_bug.cgi?id=1527565. See https://rancher.com/docs/rke/latest/en/installation/os#redhat-enterprise-linux-rhel-centos[Operating System Requirements] for RHEL/CentOS specific requirements.
====
...
* *Recommended:* Review xref:../../../../../how-to-guides/new-user-guides/backup-restore-and-disaster-recovery/back-up-rancher-launched-kubernetes-clusters.adoc[Creating Backups--High Availability Back Up and Restoration] to learn how to backup your Rancher Server in case of a disaster scenario.
* Create a Kubernetes cluster: xref:../../../../../how-to-guides/new-user-guides/kubernetes-clusters-in-rancher-setup/kubernetes-clusters-in-rancher-setup.adoc[Creating a Cluster].
