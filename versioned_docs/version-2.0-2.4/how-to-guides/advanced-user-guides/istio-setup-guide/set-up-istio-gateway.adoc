...
. Under *Enable Ingress Gateway,* click *True.* The default type of service for the Istio gateway is NodePort. You can also configure it as a xref:../../new-user-guides/kubernetes-resources-setup/load-balancer-and-ingress-controller/layer-4-and-layer-7-load-balancing.adoc[load balancer.]
