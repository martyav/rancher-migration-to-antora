...
Kubernetes cluster. If you are installing Rancher on a single node, the main architecture recommendation that applies to your installation is that the cluster running Rancher should be <<separation-of-rancher-and-user-clusters,separate from downstream clusters.>>
...
The option to install Rancher on a K3s cluster is a feature introduced in Rancher v2.4. K3s is easy to install, with half the memory of Kubernetes, all in a binary less than 100 MB.+++<figcaption>+++Architecture of a K3s Kubernetes Cluster Running the Rancher Management Server+++</figcaption>+++
