...
| `addLocal`
| "true"
| `string` - Have Rancher detect and import the "local" Rancher server cluster.  _Note: This option is no longer available in v2.5.0. In v2.5.0, the `restrictedAdmin` option is used to prevent users from modifying the local cluster._
|
...
By default enabling Audit Logging will create a sidecar container in the Rancher pod. This container (`rancher-audit-log`) will stream the log to `stdout`. You can collect this log as you would any container log. When using the sidecar as the audit log destination, the `hostPath`, `maxAge`, `maxBackups`, and `maxSize` options do not apply. It's advised to use your OS or Docker daemon's log rotation features to control disk space use. Enable the xref:../../explanations/integrations-in-rancher/cluster-logging/cluster-logging.adoc[Logging service under Rancher Tools] for the Rancher server cluster or System Project.
...
* xref:../../getting-started/installation-and-upgrade/advanced-options/advanced-use-cases/air-gap-helm2/air-gap-helm2.adoc[Air Gap: Kubernetes Install]
...
NOTE: If you are using a Private CA signed certificate, add `--set privateCA=true` and see link:../../getting-started/installation-and-upgrade/resources/add-tls-secrets.adoc#using-a-private-ca-signed-certificate[Adding TLS Secrets - Using a Private CA Signed Certificate] to add the CA cert for Rancher.
