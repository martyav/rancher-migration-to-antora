 ...
 ** You may also need to allow traffic on port 10254 for each of your nodes, if link:../../../integrations-in-rancher/monitoring-and-alerting/how-monitoring-works.adoc#pushprox[PushProx] is disabled (`ingressNginx.enabled` set to `false`), or you've upgraded from a previous Rancher version that had v1 monitoring already installed.
