...
管理员需要在迁移下述端点之前创建一个 xref:../../../new-user-guides/backup-restore-and-disaster-recovery/back-up-rancher.adoc[Rancher 备份]。
====
...
在离线环境中，由于 Graph Endpoint URL 正在更改，因此管理员需要确保其端点被<<3.2,列入白名单>>。

==== 回滚迁移

如果你需要回滚迁移，请注意以下事项：

. 如果管理员想要回滚，我们建议他们使用正确的恢复流程。有关参考信息，请参阅xref:../../../new-user-guides/backup-restore-and-disaster-recovery/back-up-rancher.adoc[备份文档]、xref:../../../new-user-guides/backup-restore-and-disaster-recovery/restore-rancher.adoc[恢复文档]和xref:../../../../reference-guides/backup-restore-configuration/examples.adoc[示例]。
 ...
 ** 使用另一个第三方身份认证系统并在 Rancher 中进行设置。请参阅xref:../../../../pages-for-subheaders/authentication-config.adoc[身份验证文档]，了解如何配置其他开放式身份验证提供程序。
____
