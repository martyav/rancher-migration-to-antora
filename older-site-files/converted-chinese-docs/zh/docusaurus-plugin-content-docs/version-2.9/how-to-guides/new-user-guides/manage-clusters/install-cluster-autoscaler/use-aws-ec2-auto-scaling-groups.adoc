 ...
 ** 安全组：`K8sMasterSg`。详情请参见 link:../../../../getting-started/installation-and-upgrade/installation-requirements/port-requirements.adoc#下游-kubernetes-集群节点[RKE 端口（自定义节点选项卡）]
 ** 标签：
...
* 安全组：`K8sWorkerSg`。详情请参见 link:../../../../getting-started/installation-and-upgrade/installation-requirements/port-requirements.adoc#下游-kubernetes-集群节点[RKE 端口（自定义节点选项卡）]
* 标签：
 ** `kubernetes.io/cluster/<clusterID>: owned`
...
详情请参见 xref:../../../new-user-guides/kubernetes-clusters-in-rancher-setup/set-up-cloud-providers/amazon.adoc[AWS 上的 RKE 集群] 和 https://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/cloudprovider/aws/README.md[AWS 上的 Cluster Autoscaler]。

=== 3. 部署节点

我们配置 AWS 后，我们需要创建虚拟机来引导集群：

* master (etcd+controlplane)：根据需要部署三个适当大小的 master 实例。详情请参见xref:../../../../pages-for-subheaders/checklist-for-production-ready-clusters.adoc[生产就绪集群的建议]。
