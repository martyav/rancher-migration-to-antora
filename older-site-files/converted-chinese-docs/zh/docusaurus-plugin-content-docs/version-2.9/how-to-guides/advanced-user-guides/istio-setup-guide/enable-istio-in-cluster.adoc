...
. 可选：为 Istio 组件配置成员访问和xref:../../../integrations-in-rancher/istio/cpu-and-memory-allocations.adoc[资源限制]。确保你的 Worker 节点上有足够的资源来启用 Istio。
. 可选：如果需要，对 values.yaml 进行额外的配置更改。
. 可选：通过link:../../../pages-for-subheaders/configuration-options.adoc#覆盖文件[覆盖文件]来添加其他资源或配置。
. 单击**安装**。
