...
一些测试会被标记为"`不适用`"。由于 Rancher 配置 RKE 集群的方式，这些测试不会在任何 CIS 扫描中运行。有关如何审核测试结果，以及为什么某些测试会被标记为不适用，请参阅 Rancher 的 Kubernetes 对应版本的link:../../reference-guides/rancher-security/rancher-security.adoc#CIS-Benchmark-和自我评估[自测指南]。
...
请参阅xref:../../reference-guides/rancher-security/rancher-security.adoc[集群加固指南中的表格]，以了解 Kubernetes、Benchmark、Rancher 以及我们的集群强化指南的版本对应关系。另外，请参阅强化指南，以获取符合 CIS 的集群的配置文件以及修复失败测试的信息。
...
要通过 "`Hardened`" 配置文件，你需要遵从link:../../reference-guides/rancher-security/rancher-security.adoc#Rancher-加固指南[强化指南]并使用强化指南中定义的 `cluster.yml` 来配置一个强化集群。
...
有关要跳过和不适用的测试列表，请参阅xref:../../how-to-guides/advanced-user-guides/cis-scan-guides/skip-tests.adoc[此页面]。
...
有关权限的详细信息，请参阅xref:rbac-for-cis-scans.adoc[此页面]。

== 配置

有关为扫描、配置文件和 Benchmark 版本配置自定义资源的更多信息，请参阅xref:configuration-reference.adoc[此页面]。

== 操作指南

要了解如何运行 CIS 扫描，请参阅 xref:../../how-to-guides/advanced-user-guides/cis-scan-guides/cis-scan-guides.adoc[CIS 扫描指南]。
