...
____
*注意*：有关配置以下选项的更多信息，请参阅xref:./backup-configuration.adoc[备份配置参考页面]。
____
...
这个例子表明，如果运行 `rancher-backup` 的节点拥有这些link:backup-configuration.adoc#ec2-节点访问-s3-的-iam-权限[访问 S3 的权限]，就不必提供 AWS 的凭证密文来创建备份。
...
____
*注意*：有关配置以下选项的更多信息，请参阅xref:./restore-configuration.adoc[恢复配置参考页面]。
____
...
这个例子表明，如果运行 `rancher-backup` 的节点拥有这些link:backup-configuration.adoc#ec2-节点访问-s3-的-iam-权限[访问 S3 的权限]，就不必提供 AWS 的凭证密文来从备份中还原。
