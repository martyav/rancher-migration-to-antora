 ...
 ** 如果 link:../../../integrations-in-rancher/monitoring-and-alerting/how-monitoring-works.adoc#pushprox[PushProx] 被禁用（`ingressNginx.enabled` 设置为 `false`），或者你已经升级了安装了 Monitoring V1 的 Rancher 版本，你可能还需要为每个节点允许端口 10254 上的流量。
* 确保你的集群满足资源要求。集群应至少有 1950Mi 可用内存、2700m CPU 和 50Gi 存储。有关资源限制和请求的详细信息，请参阅link:../../../reference-guides/monitoring-v2-configuration/helm-chart-options.adoc#配置资源限制和请求[配置资源限制和请求]。
...
. 按照xref:../../new-user-guides/kubernetes-resources-setup/secrets.adoc[此页面]上的步骤创建密文，以便将 SSL 用于告警。
