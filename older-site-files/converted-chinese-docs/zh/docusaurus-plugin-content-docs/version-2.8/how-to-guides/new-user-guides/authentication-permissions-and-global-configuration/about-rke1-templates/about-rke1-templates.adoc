...
现有集群的设置可以link:apply-templates.adoc#将现有集群转换为使用-rke-模板[保存为 RKE 模板]。这会创建一个新模板并将集群设置绑定到该模板。这样，集群只有在link:manage-rke1-templates.md#更新模板[模板更新]的情况下才能link:manage-rke1-templates.adoc#升级集群以使用新的模板修订版[使用新版本的模板]进行升级。新模板也可以用来创建新集群。
