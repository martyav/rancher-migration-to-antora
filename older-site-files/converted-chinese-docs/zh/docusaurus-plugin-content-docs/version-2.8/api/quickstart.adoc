...
. link:../reference-guides/user-settings/api-keys.adoc#创建-api-密钥[创建]一个没有作用域的 Rancher API 密钥。
...
你可以使用任何兼容的工具来引用这个文件，例如 kubectl 或 https://github.com/kubernetes/client-go[client-go]。快速演示内容请参阅 <<api-kubectl-示例,kubectl 示例>>

更多有关处理更复杂证书的设置信息，请参阅<<指定-ca-证书,指定 CA 证书>>。

更多关于可用的 kubeconfig 选项，请参阅https://kubernetes.io/docs/tasks/access-application-cluster/configure-access-multiple-clusters/[上游文档]。

== API kubectl 示例

在此示例中，我们将展示如何使用 kubectl 创建一个项目，然后删除它。关于其他可用的 Rancher 资源列表，请参阅 link:./api-reference.adocx[API 参考]。
