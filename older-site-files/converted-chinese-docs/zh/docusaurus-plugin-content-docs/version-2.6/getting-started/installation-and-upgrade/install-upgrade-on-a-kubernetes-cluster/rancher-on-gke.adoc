...
如果你已经有一个 GKE Kubernetes 集群，请直接跳转到<<7-安装-ingress,安装 Ingress>>这个步骤。然后按照link:install-upgrade-on-a-kubernetes-cluster.adoc#安装-rancher-helm-chart[此处]的步骤安装 Rancher Helm Chart。
...
* 你需要至少一个在用的 IP 地址和至少 2 个 CPU 的云配额。有关 Rancher Server 的硬件要求，请参见xref:../installation-requirements/installation-requirements.adoc[本节]。
...
. 按照https://cloud.google.com/sdk/docs/install[这些步骤]安装 Cloud SDK。The Cloud SDK 包括 `gcloud` 命令行工具。不同操作系统对应的步骤有所不同。
...
Tab 使用 gcloud init::
+
1. 运行 gcloud init 并按照指示操作： ``` gcloud init ``` 如果你在远程服务器上使用 SSH，使用 --console-only 标志，以防止该命令启动浏览器。 ``` gcloud init --console-only ``` 2. 按照指示，以授权 gcloud 使用你的 Google Cloud 账户，并选择你创建的新项目。 

Tab 使用 gcloud config::
+
...
设置 DNS 的有效方法有很多。如需获取帮助，请参见 Google Cloud 文档中的https://cloud.google.com/dns/docs/records[管理 DNS 记录]部分。

== 10. 安装 Rancher Helm Chart

按照link:../../../pages-for-subheaders/install-upgrade-on-a-kubernetes-cluster.adoc#安装-rancher-helm-chart[本页]的说明安装 Rancher Helm Chart。任何 Kubernetes 发行版上安装的 Rancher 的 Helm 说明都是一样的。
...
请参阅link:../../../pages-for-subheaders/install-upgrade-on-a-kubernetes-cluster.adoc#5-根据你选择的证书选项通过-helm-安装-rancher[Helm 安装命令]了解你的证书选项。
