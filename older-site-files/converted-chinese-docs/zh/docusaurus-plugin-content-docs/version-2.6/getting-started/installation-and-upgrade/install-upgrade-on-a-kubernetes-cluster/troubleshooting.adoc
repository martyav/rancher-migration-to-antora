...
* <<检查-rancher-是否正在运行,Rancher 是否正在运行>>
* <<证书的-cn-是-kubernetes-ingress-controller-fake-certificate,证书的 Common Name（CN）是 "Kubernetes Ingress Controller Fake Certificate">>
...
此问题的最常见原因是 `canal` pod 未能建立覆盖网络。参见 <<canal-pod-显示-ready-23,canal Pod 显示 READY `2/3`>> 进行排查。
...
如果需要了解如何进行正确设置，请参见https://docs.docker.com/install/linux/linux-postinstall/#manage-docker-as-a-non-root-user[以非 root 用户身份管理 Docker]。

* 你使用的操作系统是 RedHat 或 CentOS：由于 https://bugzilla.redhat.com/show_bug.cgi?id=1527565[Bugzilla #1527565]，你不能使用 `root` 用户连接到节点。因此，你需要添加一个单独的用户并配置其访问 Docker Socket。如果需要了解如何进行正确设置，请参见https://docs.docker.com/install/linux/linux-postinstall/#manage-docker-as-a-non-root-user[以非 root 用户身份管理 Docker]。
