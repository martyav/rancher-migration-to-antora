...
本文介绍了如何在 Amazon EKS 集群上安装 Rancher。你也可以xref:../../quick-start-guides/deploy-rancher-manager/aws-marketplace.adoc[通过 AWS Marketplace 安装 Rancher]。

如果你已经有一个 EKS Kubernetes 集群，请直接跳转到<<5-安装-ingress,安装 Ingress>>这个步骤。然后按照link:install-upgrade-on-a-kubernetes-cluster.adoc#安装-rancher-helm-chart[此处]的步骤安装 Rancher Helm Chart。
...
* IAM 用户需要具备https://eksctl.io/usage/minimum-iam-policies/[eksctl 文档]中描述的最低 IAM 策略。
====
...
* *AWS CLI v2*：如需获取帮助，请参见https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html[安装步骤]。
* *eksctl*：如需获取帮助，请参见https://docs.aws.amazon.com/eks/latest/userguide/eksctl.html[安装步骤]。
* *kubectl*：如需获得帮助，请参见https://docs.aws.amazon.com/eks/latest/userguide/install-kubectl.html[安装步骤]。
...
运行以下命令创建一个 EKS 集群。使用适用于你的用例的 AWS 区域。在选择 Kubernetes 版本时，请务必先查阅https://rancher.com/support-matrix/[支持矩阵]，以找出已针对你的 Rancher 版本验证的最新 Kubernetes 版本。

*注意*：如果你要从旧的 Kubernetes 版本更新到 Kubernetes v1.22 或更高版本，你还需要https://kubernetes.github.io/ingress-nginx/user-guide/k8s-122-migration/[更新] ingress-nginx。
...
设置 DNS 的有效方法有很多。如需获得帮助，请参见 AWS 文档中心的https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/routing-to-elb-load-balancer.html[转发流量到 ELB 负载均衡器]。

=== 8. 安装 Rancher Helm Chart

按照link:install-upgrade-on-a-kubernetes-cluster.adoc#安装-rancher-helm-chart[本页]的说明安装 Rancher Helm Chart。任何 Kubernetes 发行版上安装的 Rancher 的 Helm 说明都是一样的。
...
请参阅link:install-upgrade-on-a-kubernetes-cluster.adoc#5-根据你选择的证书选项通过-helm-安装-rancher[Helm 安装命令]了解你的证书选项。
