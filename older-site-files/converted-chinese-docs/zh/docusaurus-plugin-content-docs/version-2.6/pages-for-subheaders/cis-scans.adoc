...
https://learn.cisecurity.org/benchmarks[这里]。
...
一些测试会被标记为"`不适用`"。由于 Rancher 配置 RKE 集群的方式，这些测试不会在任何 CIS 扫描中运行。有关如何审核测试结果，以及为什么某些测试会被标记为不适用，请参阅 Rancher 的 Kubernetes 对应版本的link:./rancher-security.adoc#cis-benchmark-和自我评估[自测指南]。
...
请参阅xref:./rancher-security.adoc[集群加固指南中的表格]，以了解 Kubernetes、Benchmark、Rancher 以及我们的集群强化指南的版本对应关系。另外，请参阅强化指南，以获取符合 CIS 的集群的配置文件以及修复失败测试的信息。
...
要通过 "`Hardened`" 配置文件，你需要遵从link:./rancher-security.adoc#rancher-强化指南[强化指南]并使用强化指南中定义的 `cluster.yml` 来配置一个强化集群。
...
有关权限的详细信息，请参阅xref:../integrations-in-rancher/cis-scans/rbac-for-cis-scans.adoc[此页面]。

== 配置

有关为扫描、配置文件和 Benchmark 版本配置自定义资源的更多信息，请参阅xref:../integrations-in-rancher/cis-scans/configuration-reference.adoc[此页面]。

== 操作指南

请参阅xref:../pages-for-subheaders/cis-scan-guides.adoc[此处]了解 CIS 扫描的操作指南。
