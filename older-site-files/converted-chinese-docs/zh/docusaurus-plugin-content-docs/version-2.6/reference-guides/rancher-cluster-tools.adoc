...
有关详细信息，请参阅 xref:../pages-for-subheaders/logging.adoc[Logging 文档]。
...
如需更多信息，请参阅xref:../pages-for-subheaders/monitoring-and-alerting.adoc[监控文档]。
...
如需更多信息，请参阅 xref:../pages-for-subheaders/istio.adoc[Istio 文档]。

== OPA Gatekeeper

https://github.com/open-policy-agent/gatekeeper[OPA Gatekeeper] 是一个开源项目，它对 OPA 和 Kubernetes 进行了集成，以通过许可控制器 Webhook 提供策略控制。有关如何在 Rancher 中启用 Gatekeeper 的详细信息，请参阅 xref:../integrations-in-rancher/opa-gatekeeper.adoc[OPA Gatekeeper]。

== CIS 扫描

Rancher 可以通过运行安全扫描来检查 Kubernetes 是否按照 CIS Kubernetes Benchmark 中定义的安全最佳实践进行部署。

如需更多信息，请参阅 xref:../pages-for-subheaders/cis-scan-guides.adoc[CIS 扫描文档]。
