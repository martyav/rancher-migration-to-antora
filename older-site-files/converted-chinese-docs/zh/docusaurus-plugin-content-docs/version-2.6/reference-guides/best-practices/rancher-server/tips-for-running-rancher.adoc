...
如果你在 vSphere 环境中安装 Rancher，请参见xref:on-premises-rancher-in-vsphere.adoc[最佳实践文档]。
...
在部署节点时，请遵循 K8s 和 etcd 的最佳实践，其中包括禁用 swap，检查集群中的所有主机之间是否有良好的网络连接，为每个节点使用唯一的主机名、MAC 地址和 `product_uuids`，检查所需端口是否已经打开，并使用配置 SSD 的 etcd 进行部署。详情请参见 https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/install-kubeadm/#before-you-begin[kubernetes 官方文档]和 https://etcd.io/docs/v3.4/op-guide/performance/[etcd 性能操作指南]。
...
Rancher Server 的 Kubernetes 集群应该尽可能满足xref:../../../pages-for-subheaders/installation-requirements.adoc[系统和硬件要求]。越偏离系统和硬件要求，你可能面临的风险就越大。
...
在集群中xref:../../../pages-for-subheaders/monitoring-and-alerting.adoc[启用监控]后，你可以通过设置告警通知，来了解集群容量的使用情况。你还可以使用 Prometheus 和 Grafana 监控框架，在你扩容时建立关键指标的基线。
