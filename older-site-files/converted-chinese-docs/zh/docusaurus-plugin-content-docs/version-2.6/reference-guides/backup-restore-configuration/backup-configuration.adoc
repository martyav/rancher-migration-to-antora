...
rancher-backup 通过调用 kube-apiserver 来收集资源。apiserver 返回的对象会被解密，所以即使启用了https://kubernetes.io/docs/tasks/administer-cluster/encrypt-data/[静态加密]，备份收集的加密对象也会是明文。
...
例如，xref:../../how-to-guides/new-user-guides/backup-restore-and-disaster-recovery/migrate-rancher-to-new-cluster.adoc[将 Rancher 迁移到新集群]时，该文件可用于在新集群​​中重新创建 secret。
====
...
. *凭证密文*（可选）：如果你需要使用 AWS 访问密钥（access key）和密文密钥（secret key）来访问 S3 存储桶，请使用带有密钥和指令 `accessKey` 和 `secretKey` 的凭证来创建密文。它可以是任意一个命名空间。你可以点击<<credentialsecret-示例,此处>>查看示例密文。如果运行 operator 的节点在 EC2 中，并且设置了允许它们访问 S3 的 IAM 权限，则此指令是不必要的（如<<ec2-节点访问-s3-的-iam-权限,本节>>所述）。凭证密文下拉菜单列出了所有命名空间的密文。
...
. *端点*：用于访问存储桶区域中的 S3 的https://docs.aws.amazon.com/general/latest/gr/s3.html[端点]。
. *端点 CA*（可选）：Base64 编码的 CA 证书。如需获取示例，请参见 <<s3-存储配置示例,S3 兼容配置示例>>。
...
| 如果你需要使用 AWS 访问密钥（access key）和密文密钥（secret key）来访问 S3 存储桶，请使用带有密钥和指令 `accessKey` 和 `secretKey` 的凭证来创建密文。它可以在任何命名空间中，只要你在 `credentialSecretNamespace` 中提供该命名空间。点击<<credentialsecret-示例,这里>>查看示例密文。如果运行 operator 的节点在 EC2 中，并且设置了允许它们访问 S3 的 IAM 权限，则此指令是不必要的（如<<ec2-节点访问-s3-的-iam-权限,本节>>所述）。
|

| `credentialSecretNamespace`
| 包含访问 S3 的凭证的密文的命名空间。如果运行 operator 的节点在 EC2 中，并且设置了允许它们访问 S3 的 IAM 权限，则此指令是不必要的（如<<ec2-节点访问-s3-的-iam-权限,本节>>所述）。
|
...
| `endpoint`
| 用于访问存储桶区域中的 S3 的https://docs.aws.amazon.com/general/latest/gr/s3.html[端点]。
| ✓

| `endpointCA`
| Base64 编码的 CA 证书。如需获取示例，请参见 <<s3-存储配置示例,S3 兼容配置示例>>。
|
...
如需获取 Backup 自定义资源的示例，请参见link:examples.adoc#备份[本页]。
