...
使用xref:workloads-and-pods/workloads-and-pods.adoc[工作负载]将应用部署到集群节点，工作负载是包含用于运行应用的 pod 的对象，以及为部署行为设置规则的元数据。工作负载可以部署在集群范围内，也可以部署在一个命名空间内。
...
* 将工作负载xref:workloads-and-pods/upgrade-workloads.adoc[升级]到它运行的应用的更新版本。
* 如果升级出现问题，将工作负载xref:workloads-and-pods/roll-back-workloads.adoc[回滚]到以前的版本。
* xref:workloads-and-pods/add-a-sidecar.adoc[添加一个 sidecar]，这是一个支持主要工作负载的工作负载。
...
* link:load-balancer-and-ingress-controller/layer-4-and-layer-7-load-balancing.adoc#layer-4-load-balancer#四层负载均衡器[Layer-4 负载均衡器]
* link:load-balancer-and-ingress-controller/layer-4-and-layer-7-load-balancing.adoc#七层负载均衡器[Layer-7 负载均衡器]

有关详细信息，请参阅xref:load-balancer-and-ingress-controller/layer-4-and-layer-7-load-balancing.adoc[负载均衡器]。
...
有关详细信息，请参阅 xref:load-balancer-and-ingress-controller/add-ingresses.adoc[Ingress]。
...
有关详细信息，请参阅xref:create-services.adoc[服务发现]。

== 流水线

在将您的项目link:../../../reference-guides/pipelines/pipelines.adoc#1-配置版本控制提供程序[配置到版本控制提供商]之后，您可以添加仓库并开始为每个仓库配置流水线。

更多信息，请xref:../../../reference-guides/pipelines/pipelines.adoc[参阅]。
...
* xref:encrypt-http-communication.adoc[证书]：用于加密/解密进入或离开集群的数据的文件。
* xref:configmaps.adoc[ConfigMap]：存储一般配置信息的文件，例如一组配置文件。
* xref:secrets.adoc[密文]：存储密码、token 或密钥等敏感数据的文件。
* xref:kubernetes-and-docker-registries.adoc[镜像仓库]：携带用于验证私有镜像仓库的凭证的文件。
