...
为了使用模板创建新的虚拟机，Rancher 有虚拟机必须预先安装的<<要求,特定要求>>。根据这些要求配置虚拟机后，你需要<<准备虚拟机,准备虚拟机>>，然后再<<创建模板,创建模板>>。准备工作完成后，虚拟机可以<<转换为模板,转换为模板>>并<<移动到内容库,移动到内容库中>>，然后 Rancher 节点池就可以使用它了。
...
你可以手动创建虚拟机，也可以使用<<手动创建的替代方案,其他替代方法>>来创建虚拟机。
