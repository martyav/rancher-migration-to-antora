...
如果你已使用 "`Rancher Setup`" AWS Marketplace 产品安装了 Rancher，请跳至<<4-创建-oidc-提供程序,步骤 4>>。
...
按照 xref:../../../getting-started/installation-and-upgrade/install-upgrade-on-a-kubernetes-cluster/rancher-on-amazon-eks.adoc[Rancher 文档]创建 EKS 集群。进行到link:../../../getting-started/installation-and-upgrade/install-upgrade-on-a-kubernetes-cluster/rancher-on-amazon-eks.adoc#8-安装-rancher-helm-chart[安装 Rancher Helm Chart]（最后一步）时，*停止并返回此页面*。该集群需要满足以下要求：
...
除了在 link:../../../getting-started/installation-and-upgrade/install-upgrade-on-a-kubernetes-cluster/rancher-on-amazon-eks.adoc#8-安装-rancher-helm-chart[Rancher 文档]中指定的 Rancher 安装选项外，你还需要启用其它指标。
