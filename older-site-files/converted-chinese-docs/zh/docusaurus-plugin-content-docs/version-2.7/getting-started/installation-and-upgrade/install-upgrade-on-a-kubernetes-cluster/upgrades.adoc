...
有关使用 Docker 安装的 Rancher 的升级说明，请参见xref:../other-installation-methods/rancher-on-a-single-node-with-docker/upgrade-docker-installed-rancher.adoc[本页。]
...
不支持 _升级_ 或 _升级到_ link:../resources/choose-a-rancher-version.adoc#helm-chart-仓库[rancher-alpha 仓库]中的任何 Chart。

=== Helm 版本+++<DeprecationHelm2>++++++</DeprecationHelm2>+++
...
xref:../other-installation-methods/air-gapped-helm-cli-install/air-gapped-helm-cli-install.adoc[仅适用于离线安装]：为新的 Rancher Server 版本收集和推送镜像。使用你需要针对 Rancher 版本升级的镜像，按照步骤xref:../other-installation-methods/air-gapped-helm-cli-install/publish-images.adoc[推送镜像到私有镜像仓库]。
...
使用xref:../../../how-to-guides/new-user-guides/backup-restore-and-disaster-recovery/back-up-rancher.adoc[备份应用]来备份 Rancher。
...
+
关于仓库及其区别，请参见 link:../resources/choose-a-rancher-version.adoc#helm-chart-仓库[Helm Chart 仓库]。
...
====
+
如果你想切换到不同的 Helm Chart 仓库，请按照link:../resources/choose-a-rancher-version.adoc#切换到不同-helm-chart-仓库[切换仓库步骤]进行操作。如果你要切换仓库，请先再次列出仓库，再继续执行步骤 3，以确保添加了正确的仓库。
+
====
...
如果你在离线环境中安装 Rancher，请跳过本页的其余部分，按照xref:air-gapped-upgrades.adoc[本页]上的说明渲染 Helm 模板。
====
