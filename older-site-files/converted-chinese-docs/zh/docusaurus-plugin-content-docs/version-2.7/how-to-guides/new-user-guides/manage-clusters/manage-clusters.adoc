...
将集群xref:../kubernetes-clusters-in-rancher-setup/kubernetes-clusters-in-rancher-setup.adoc[配置到 Rancher] 之后，link:../authentication-permissions-and-global-configuration/manage-role-based-access-control-rbac/cluster-and-project-roles.adoc#cluster-roles[集群所有者]需要管理这些集群。管理集群的选项如下：
