...
有关 Rancher 如何工作的详情（与安装方法无关），请参见xref:../../../pages-for-subheaders/rancher-manager-architecture.adoc[架构]。
...
* Ingress Controller 会把流量转发到 Rancher deployment 的 Pod 上的 TCP/80 端口。+++<figcaption>+++使用 4 层负载均衡器在 Kubernetes 集群中安装 Rancher：Ingress Controller 的 SSL 终止：+++</figcaption>+++

image:/img/ha/rancher2ha.svg[Rancher 的高可用 Kubernetes 安装]
