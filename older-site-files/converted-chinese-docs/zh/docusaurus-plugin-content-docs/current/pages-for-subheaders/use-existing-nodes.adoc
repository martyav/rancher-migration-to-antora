...
根据xref:../how-to-guides/new-user-guides/kubernetes-clusters-in-rancher-setup/node-requirements-for-rancher-managed-clusters.adoc[安装要求]和xref:checklist-for-production-ready-clusters.adoc[生产就绪集群的检查清单]配置主机。
 ...
 ** 请参阅xref:use-windows-clusters.adoc[启用 Windows 支持选项]。
...
. 从**节点角色**中，选择要由集群节点充当的角色。你必须为 `etcd`、`worker` 和 `controlplane` 角色配置至少一个节点。自定义集群需要所有三个角色才能完成配置。有关角色的详细信息，请参阅link:../reference-guides/kubernetes-concepts.adoc#kubernetes-集群中节点的角色[本节]。

[NOTE]
====

* 使用 Windows 主机作为 Kubernetes Worker 节点？请参阅xref:use-windows-clusters.adoc[本节]。
