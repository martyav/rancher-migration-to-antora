...
| `agentTLSMode`
| ""
| `string` - either `system-store` or `strict`. See link:./tls-settings.adoc#agent-tls-enforcement[Agent TLS Enforcement]
|
...
| `restrictedAdmin`
| `false`
| `bool` - When this option is set to `true`, the initial Rancher user has restricted access to the local Kubernetes cluster to prevent privilege escalation. For more information, see the section about the link:../../../how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/manage-role-based-access-control-rbac/global-permissions.adoc#restricted-admin[restricted-admin role.] _Note: this option is deprecated, and may be removed in v2.10.0 or later.
|
