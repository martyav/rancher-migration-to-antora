...
For information on enabling experimental features, refer to xref:../../getting-started/installation-and-upgrade/advanced-options/enable-experimental-features/enable-experimental-features.adoc[this page.]
...
| `addLocal`
| "true"
| `string` - Have Rancher detect and import the "local" Rancher server cluster.  _Note: This option is no longer available in v2.5.0. Consider using the `restrictedAdmin` option to prevent users from modifying the local cluster._
|
...
| `auditLog.level`
| 0
| `int` - set the xref:../../getting-started/installation-and-upgrade/advanced-options/advanced-use-cases/enable-api-audit-log.adoc[API Audit Log] level. 0 is off. [0-3]
|
...
| `busyboxImage`
| "busybox"
| `string` - Image location for busybox image used to collect audit logs
|
...
| `ingress.enabled`
| true
| When set to false, Helm will not install a Rancher ingress. Set the option to false to deploy your own ingress. _Available as of v2.5.6_
|
...
| `replicas`
| 3
| `int` - Number of replicas of Rancher pods
|
...
Enabling the xref:../../getting-started/installation-and-upgrade/advanced-options/advanced-use-cases/enable-api-audit-log.adoc[API Audit Log].

You can collect this log as you would any container log. Enable xref:../../explanations/integrations-in-rancher/logging/logging.adoc[logging] for the `System` Project on the Rancher server cluster.
...
For details on installing Rancher with a private registry, see xref:../../getting-started/installation-and-upgrade/other-installation-methods/air-gapped-helm-cli-install/air-gapped-helm-cli-install.adoc[Air Gapped Helm CLI Install].
...
NOTE: If you are using a Private CA signed certificate, add `--set privateCA=true` and see xref:../../getting-started/installation-and-upgrade/resources/add-tls-secrets.adoc[Adding TLS Secrets - Using a Private CA Signed Certificate] to add the CA cert for Rancher.
