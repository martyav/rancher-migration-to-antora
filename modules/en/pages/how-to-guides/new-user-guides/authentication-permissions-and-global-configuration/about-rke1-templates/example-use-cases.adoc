...
+++<head>++++++<link rel="canonical" href="https://ranchermanager.docs.rancher.com/how-to-guides/new-user-guides/authentication-permissions-and-global-configuration/about-rke1-templates/example-use-cases">++++++</link>++++++</head>+++
...
. First, an administrator turns on link:enforce-templates.md#requiring-new-clusters-to-use-an-rke-template[RKE template enforcement.] This means that every xref:../manage-role-based-access-control-rbac/global-permissions.adoc[standard user] in Rancher will need to use an RKE template when they create a cluster.
...
To share ownership of the template with Bob, Alice link:access-or-share-templates.md#sharing-ownership-of-templates[adds Bob as an owner of her template.]
...
* link:manage-rke1-templates.md#updating-a-template[Revise the template] when the best practices change
* link:manage-rke1-templates.md#disabling-a-template-revision[Disable outdated revisions] of the template so that no new clusters can be created with it
* link:manage-rke1-templates.md#deleting-a-template[Delete the whole template] if the organization wants to go in a different direction
* link:manage-rke1-templates.md#setting-a-template-revision-as-default[Set a certain revision as default] when users create a cluster with it. End users of the template will still be able to choose which revision they want to create the cluster with.
