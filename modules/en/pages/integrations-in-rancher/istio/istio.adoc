...
+++<head>++++++<link rel="canonical" href="https://ranchermanager.docs.rancher.com/integrations-in-rancher/istio">++++++</link>++++++</head>+++
...
Additional Istio Ingress gateways can be enabled via the link:configuration-options/configuration-options.md#overlay-file[overlay file].

=== Egress Support

By default the Egress gateway is disabled, but can be enabled on install or upgrade through the values.yaml or via the link:configuration-options/configuration-options.md#overlay-file[overlay file].
